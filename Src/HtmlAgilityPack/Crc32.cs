...
namespace HtmlAgilityPack
{
  ...
  public class Crc32
  {
    ...
    public static uint CRC32Bytes(byte[] bytes)
    {
      ...
      {
        --length;
        crc = Crc32.UPDC32(bytes[length], crc);
        ++num;
      }
      return ~crc;
    }
    ...
    public static uint CRC32String(string text)
    {
      ...
      {
        --length;
        int num2 = (int) text[length];
        crc = Crc32.UPDC32((byte) (num2 & (int) byte.MaxValue), Crc32.UPDC32((byte) (num2 >> 8), crc));
        ++num1;
      }
      return ~crc;
    }

    internal uint AddToCRC32(int c) => this.AddToCRC32((ushort) c);

    internal uint AddToCRC32(ushort c)
    {
      byte octet = (byte) ((uint) c & (uint) byte.MaxValue);
      this._crc32 = Crc32.UPDC32((byte) ((uint) c >> 8), this._crc32);
    ...
    {
      return Crc32.crc_32_tab[((int) crc ^ (int) octet) & (int) byte.MaxValue] ^ crc >> 8;
    }
  }
}
