...
namespace HtmlAgilityPack
{
  public class HtmlEntity
  {
    ...
    public static Dictionary<int, string> EntityName => HtmlEntity._entityName;

    public static Dictionary<string, int> EntityValue => HtmlEntity._entityValue;
                    ...
                    else
                    {
                      if (stringBuilder2[0] == '#')
                      {
                        ...
                        try
                        {
                          ...
                          int fromBase;
                          if (str2.StartsWith("x", StringComparison.OrdinalIgnoreCase))
                          {
                            fromBase = 16;
                      ...
                      {
                        int num;
                        if (!HtmlEntity._entityValue.TryGetValue(stringBuilder2.ToString(), out num))
                          stringBuilder1.Append("&" + (object) stringBuilder2 + ";");
                        else
    ...
    {
      HtmlNode htmlNode = node != null ? node.CloneNode(true) : throw new ArgumentNullException(nameof (node));
    ...
    public static string Entitize(string text) => HtmlEntity.Entitize(text, true);
    ...
    public static string Entitize(string text, bool useNames, bool entitizeQuotAmpAndLtGt)
    {
      switch (text)
      {
        ...
        default:
          ...
          for (int index = 0; index < text.Length; ++index)
          {
            int key = (int) text[index];
            if (key > (int) sbyte.MaxValue || entitizeQuotAmpAndLtGt && (key == 34 || key == 38 || key == 60 || key == 62))
            {
              string str;
              HtmlEntity.EntityName.TryGetValue(key, out str);
