...
// Type: App1uwp.Utils.ColorExtensions
...
namespace App1uwp.Utils
{
  public static class ColorExtensions
  {
    ...
    {
      return ColorExtensions.FromString(c, false, out color);
    }
    ...
    private static bool FromString(string c, bool throwOnFail, out Color color)
    {
      ...
      else
      {
        ...
        PropertyInfo declaredProperty = typeof (Colors).GetTypeInfo().GetDeclaredProperty(c);
    ...
    {
      return (Color) XamlReader.Load(string.Format("<Color xmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\">{0}</Color>", (object) c));
    }

    public static Color FromRgb(double red, double green, double blue)
    {
      return Color.FromArgb(byte.MaxValue, (byte) ((double) byte.MaxValue * red), (byte) ((double) byte.MaxValue * green), (byte) ((double) byte.MaxValue * blue));
    }
    ...
    {
      return Math.Max(Math.Max(Math.Max((byte) Math.Abs((pixel >> 24 & (int) byte.MaxValue) - (color >> 24 & (int) byte.MaxValue)), (byte) Math.Abs((pixel >> 16 & (int) byte.MaxValue) - (color >> 16 & (int) byte.MaxValue))), (byte) Math.Abs((pixel >> 8 & (int) byte.MaxValue) - (color >> 8 & (int) byte.MaxValue))), (byte) Math.Abs((pixel & (int) byte.MaxValue) - (color & (int) byte.MaxValue)));
    }

    public static List<Color> GetNamedColors()
    {
      return typeof (Colors).GetTypeInfo().DeclaredProperties.Select<PropertyInfo, Color>((Func<PropertyInfo, Color>) (pi => (Color) pi.GetValue((object) null))).ToList<Color>();
    }

    public static List<string> GetColorNames()
    {
      return typeof (Colors).GetTypeInfo().DeclaredProperties.Select<PropertyInfo, string>((Func<PropertyInfo, string>) (pi => pi.Name)).ToList<string>();
    }
  }
}
